...
abstract class BaseGameControllerNxN extends ChangeNotifier {
  ...
  void validateAll() {
    ...
    _accuracyPercentage =
        (totalPlayerCount > 0) ? (correctCount / totalPlayerCount) * 100 : 0.0;
  ...
  void _calculateScore() {
    ...
    int timeBonus = 0;
    if (mode == GameMode.timed && _correctAnswers > 0) {
    ...
    whether its hard mode or not, and if its decimal mode or not. 
  ...
  double _generateRandomNumber(Random random) {
    ...
    if (_useDecimals) {
      ...
      final int dynamicMax = _hardMode ? ABSOLUTE_MAX : (gridSize * gridSize);

      final int safeMax = dynamicMax > ABSOLUTE_MAX ? ABSOLUTE_MAX : dynamicMax;
